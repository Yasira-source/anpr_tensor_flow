...
$penalty = $_GET['penalty'];
  ...
  if(($status != $trans_status) OR ($trans_id != $id)){
     ...
     header("Location: add_penalty_payment.php");
     exit;
  }else{
      //Give value
      $conn = $pdo->open();
      $stmt = $conn->prepare("INSERT INTO `penalty_payments`
      ( `_email`, `_phone`, `_amount`, `_ref`, `_transid`, `_charge_amount`, `_penaltyid`, `_names`, `_transstatus`) VALUES 
      (:email,:phone,:amount,:ref,:trans,:charge,:penalty,:names,:status)");
  $stmt->execute(['email'=>$email,'names'=>$fullName,'phone'=>$phone,'amount'=>$amount,'penalty'=>$penalty,'email'=>$email,'status'=>$status,'ref'=>$reference,'trans'=>$id,'charge'=>$charged_amount]);
 $_SESSION['success']="Payment Completed Successfully !";
 header("Location: citizen_penalties.php");
